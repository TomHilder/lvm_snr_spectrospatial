...
class TwoComponentEmissionLine(SpectralSpatialModel):
    ...
    def __init__(
        ...
        self.line_1 = EmissionLine(
            μ=line_centre,
            A=PositiveGPField(kernel=A_kernel_1, n_modes=n_modes),
            v=GPField(kernel=v_kernel_1, n_modes=n_modes),
            vσ=PositiveGPField(kernel=σ_kernel_1, n_modes=n_modes),
        ...
        self.line_2 = EmissionLine(
            μ=line_centre,
            A=PositiveGPField(kernel=A_kernel_2, n_modes=n_modes),
            v=GPField(kernel=v_kernel_2, n_modes=n_modes),
            vσ=PositiveGPField(kernel=σ_kernel_2, n_modes=n_modes),
            σ_lsf=PerSpaxel(n_spaxels=n_spaxels, spaxel_values=σ_lsf),
            v_bary=PerSpaxel(n_spaxels=n_spaxels, spaxel_values=v_bary),
