"""two_lines.py
Spectrospatial model for two-component emission lines in LVM, where the two components blend but stay ordered in velocity. We don't assume the lines share anything in common, but we always assume line_2 is redward (or blueward, haven't decided yet) of line_1. This should help prevent component switching and improve identifiability.
NOTE: We are starting WITHOUT that constraint. We will allow the lines not to preserve their ordering in velocity.
"""
